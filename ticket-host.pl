...
# --- configuration ---
# user token is configured on selected user account in GLPI
# app_token is configured in glpi api configuration in API access entries
...
# arguments are to be passed in form name=value name2=value2 ....
# e.g hoststate="$HOSTSTATE$" ....
# let's extract them and stick them into array
# there are definitely smarter ways to do it,
...
sub insert_glpi_ticket() {
	...
	my $ticket_data = { 
		"input" => {
			"name" => "Host $arguments{'eventhost'} is unavailable.",
			"content" => "Host $arguments{'eventhost'} is unavailable.\n\r
Host \t\t\t = $arguments{'eventhost'} \r
Check Attempts \t = $arguments{'hostattempts'}/$arguments{'maxhostattempts'} \r
Check Command \t = $arguments{'hostcommand'} \r
Check Output \t = $arguments{'hostoutput'} \r
$arguments{'longhostoutput'}",
		}, 
	};
...
if ($arguments{'lasthoststate'} eq $arguments{'hoststate'}) { 
    # nothing to do, exit
    exit 0
}

if ($arguments{"hoststatetype"} eq 'SOFT') {
	# no ticket until monitoring retries
	exit 0
}
...
if ("$arguments{'hoststate'}" eq 'UP') { 
	print "Host $arguments{'eventhost'} juz dziala, zamykam ticket\n";
	close_glpi_tickets()
...
if ("$arguments{'hoststate'}" eq 'DOWN') {
	...
	if ($ile > 0) {
		print "$arguments{'eventhost'} already has $ile open tickets, skipping\n";
	} else {
		print "Host $arguments{'eventhost'} has no open tickets, making one\n";
		insert_glpi_ticket();
	}
}
